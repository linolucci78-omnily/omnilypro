-- Customer Wallet System (FIXED - rimosso check wallet_enabled)
...
CREATE POLICY customer_wallets_staff_policy ON customer_wallets
  FOR ALL USING (
    auth.uid() IN (SELECT id FROM staff WHERE organization_id = customer_wallets.organization_id)
  );
...
CREATE POLICY wallet_transactions_staff_policy ON wallet_transactions
  FOR ALL USING (
    auth.uid() IN (SELECT id FROM staff WHERE organization_id = wallet_transactions.organization_id)
  );
