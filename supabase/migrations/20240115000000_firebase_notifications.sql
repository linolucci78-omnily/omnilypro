...
DROP POLICY IF EXISTS "Staff can view organization device tokens" ON device_tokens;
CREATE POLICY "Staff can view organization device tokens"
  ON device_tokens FOR SELECT
  USING (
    organization_id IN (
...
DROP POLICY IF EXISTS "Staff can view organization notification log" ON notification_log;
CREATE POLICY "Staff can view organization notification log"
  ON notification_log FOR SELECT
  USING (
    organization_id IN (
      SELECT org_id FROM organization_users WHERE user_id = auth.uid()
...
DROP POLICY IF EXISTS "Staff can manage organization campaigns" ON push_campaigns;
CREATE POLICY "Staff can manage organization campaigns"
