...
import com.zcs.zcssdkdemo.utils.M1CardUtils;
...
public class EmvActivity extends BaseActivity {
    ...
    private NativeNfcCard nativeNfcCard;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        initView();
        initSdk();
        EmvTermParam.emvParamFilePath = mContext.getFilesDir().getPath() + "/emv/";
    }
    @Override
    protected void onPause() {
        super.onPause();
        nativeNfcCard.NativeNfcCardUnRegister(this);
    }

    @Override
    protected void onResume() {
        super.onResume();
        nativeNfcCard = mCardReadManager.getNativeNfcCard(this.mContext);
        if(nativeNfcCard.isEnabled()){
            nativeNfcCard.NativeNfcCardRegister(this, NfcAdapter.FLAG_READER_NFC_A | NfcAdapter.FLAG_READER_SKIP_NDEF_CHECK, new OnNativeNfcDetectedListener() {
                @Override
                public void onCardInfo(CardReaderTypeEnum cardReaderTypeEnum, Tag tag) {
                    realCardType = cardReaderTypeEnum;
                    ...
                    if(isIsoDep) {
                        emv(realCardType);
                    } else if(haveMifareUltralight) {
                        String[][] metaInfo = new String[40][16];
                        try {
                            metaInfo = M1CardUtils.readCard(tag);
                            Log.d(TAG,"readM1 sector 0"+"  Block 1"+" :" + metaInfo[0][1]+"\n");
                            showLog("readM1 sector 0"+"  Block 1"+" :" + metaInfo[0][1]+"\n");
                        } catch (IOException e) {
                            Log.d(TAG, "readM1 failed e = "+ e.getMessage());
                            showLog("readM1 failed e = "+ e.getMessage());
                            e.printStackTrace();
                        }
                    } else {
                        Log.d("Debug","not emv or M1 card");
                    }
    ...
    private void initView() {
        ...
        mTvLog = (TextView) findViewById(R.id.textview_result);
    ...
    private void emv(CardReaderTypeEnum cardType) {
        ...
        // 2. set params
        EmvTermParam.nativeNfcActive=0;
        if(Sys.getConnectType().equals(ConnectTypeEnum.BLUETOOTH)) {
            EmvTermParam.useCallBackApdu=1;
        }
        ...
        } else  if (cardType == CardReaderTypeEnum.RF_CARD||cardType == CardReaderTypeEnum.NATIVE_NFC_CARD) {
            if(cardType == CardReaderTypeEnum.NATIVE_NFC_CARD)
                    EmvTermParam.nativeNfcActive=1;
       ...
       // loadVisaAIDs(emvHandler);
        ...
        OnEmvListener onEmvListener = new OnEmvListener() {
            ...
            public int onInputPIN(byte pinType) {
                ...
                if(realCardType==CardReaderTypeEnum.NATIVE_NFC_CARD) return 0;
