...
const supabaseUrl = import.meta.env.VITE_SUPABASE_URL || 'https://sjvatdnvewohvswfrdiv.supabase.co'
const supabaseAnonKey = import.meta.env.VITE_SUPABASE_ANON_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InNqdmF0ZG52ZXdvaHZzd2ZyZGl2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTY3NDM0ODUsImV4cCI6MjA3MjMxOTQ4NX0.310-1eBrnWxaDYVJ2QeEhx9xmqVljTBqSDArLMjFiMk'
...
export const organizationsApi = {
  ...
  // Get ALL organizations (for admin page - no filtering)
  async getAllForAdmin(): Promise<Organization[]> {
    // Get ALL organizations without user filtering
    const { data, error } = await supabase
      .from('organizations')
      .select(`
        *,
