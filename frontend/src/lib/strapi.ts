...
// Attribute types (the actual data structure from Strapi)
interface WebsiteTemplateAttributes {
  name: string;
  slug: string;
  category?: 'ristorante' | 'bar' | 'negozio' | 'servizi' | 'beauty' | 'altro';
  description?: string;
  preview_image?: {
    data?: {
      attributes?: {
...
// Strapi entity with attributes wrapper
interface WebsiteTemplate {
  id: number;
  documentId?: string; // Strapi 5 document ID
  attributes: WebsiteTemplateAttributes;
}

interface OrganizationWebsiteAttributes {
...
interface OrganizationWebsite {
  id: number;
  documentId?: string; // Strapi 5 document ID
  attributes: OrganizationWebsiteAttributes;
}
...
class StrapiClient {
   ...
   */
  async createTemplate(data: Partial<WebsiteTemplateAttributes>): Promise<WebsiteTemplate> {
  ...
  async updateTemplate(
    id: number,
    data: Partial<WebsiteTemplateAttributes>
  ): Promise<WebsiteTemplate> {
  ...
  async createOrganizationWebsite(
    data: Partial<OrganizationWebsiteAttributes>
...
export type {
  WebsiteTemplate,
  WebsiteTemplateAttributes,
  OrganizationWebsite,
  OrganizationWebsiteAttributes,
  StrapiResponse
};
