-- ============================================
-- COMPLETE WEBSITE BUILDER MIGRATION (FINAL VERSION)
...
-- 21. RLS POLICIES - Drop existing ones first
...
-- Policy: Allow authenticated users to view submissions for their organizations
-- NOTE: Adjust this based on your actual auth structure
CREATE POLICY "Allow organization members to view submissions"
  ...
  USING (
    EXISTS (
      ...
      WHERE organizations.id = contact_form_submissions.organization_id
      AND organizations.id = ANY(
...
CREATE POLICY "Allow organization admins to update submissions"
  ...
  USING (
    EXISTS (
      ...
      WHERE organization_users.organization_id = contact_form_submissions.organization_id
