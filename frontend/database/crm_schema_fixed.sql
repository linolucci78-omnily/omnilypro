-- ============================================================================
-- OMNILY PRO - CRM SCHEMA COMPLETO E FUNZIONALE
-- Customer Relationship Management con tutte le dipendenze
-- ============================================================================

-- ============================================================================
-- 0. TABELLE BASE NECESSARIE (se non esistono)
-- ============================================================================

-- Tabella per associare utenti alle organizzazioni
CREATE TABLE IF NOT EXISTS organization_users (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid REFERENCES auth.users(id) ON DELETE CASCADE,
  org_id uuid REFERENCES organizations(id) ON DELETE CASCADE,
  role varchar(50) NOT NULL DEFAULT 'member', -- super_admin, org_admin, manager, cashier, member
  ...
  CONSTRAINT organization_users_unique UNIQUE (user_id, org_id)
);
...
CREATE INDEX IF NOT EXISTS idx_organization_users_org ON organization_users(org_id);
...
-- Policy per ORGANIZATION_USERS
CREATE POLICY "Admin full access organization_users" ON organization_users FOR ALL USING (
  auth.uid() IN (
...
CREATE POLICY "Users see own organization_users" ON organization_users FOR SELECT USING (
  user_id = auth.uid() OR
  org_id IN (
...
-- Trigger per organization_users
...
SELECT
  'Mario', 'Rossi', 'mario.rossi@email.com', id, 1500.00, 'Gold', 'vip', 85.5
FROM organizations
LIMIT 1
...
INSERT INTO customers (first_name, last_name, email, organization_id, total_spent, tier, status, engagement_score)
SELECT
  'Anna', 'Verdi', 'anna.verdi@email.com', id, 850.00, 'Silver', 'active', 72.3
FROM organizations
LIMIT 1
ON CONFLICT (email) DO NOTHING;
...
INSERT INTO customers (first_name, last_name, email, organization_id, total_spent, tier, status, engagement_score)
SELECT
  'Luca', 'Bianchi', 'luca.bianchi@email.com', id, 300.00, 'Bronze', 'active', 45.8
FROM organizations
LIMIT 1
ON CONFLICT (email) DO NOTHING;
...
INSERT INTO customers (first_name, last_name, email, organization_id, total_spent, tier, status, engagement_score)
SELECT
  'Giulia', 'Neri', 'giulia.neri@email.com', id, 2100.00, 'Platinum', 'vip', 92.1
FROM organizations
LIMIT 1
ON CONFLICT (email) DO NOTHING;
...
INSERT INTO customers (first_name, last_name, email, organization_id, total_spent, tier, status, engagement_score)
SELECT
  'Francesco', 'Romano', 'francesco.romano@email.com', id, 120.00, 'Bronze', 'inactive', 23.4
FROM organizations
LIMIT 1
ON CONFLICT (email) DO NOTHING;
...
INSERT INTO customer_segments (name, description, organization_id, criteria, customer_count)
SELECT
  'Clienti Attivi', 'Clienti con engagement alto', id, '{"engagement_score": {"min": 70}}', 0
FROM organizations
LIMIT 1
ON CONFLICT (name, organization_id) DO NOTHING;
...
SELECT
  'Campagna Estate 2024', 'Promozione estiva per tutti i clienti', id, 'email', 'completed', 1250, 487, 123
FROM organizations
LIMIT 1;

INSERT INTO marketing_campaigns (name, description, organization_id, type, status, sent_count, opened_count, clicked_count)
SELECT
  'Black Friday 2024', 'Offerte speciali Black Friday', id, 'email', 'running', 890, 312, 89
FROM organizations
LIMIT 1;

-- ============================================================================
-- FINE SCHEMA CRM COMPLETO E FUNZIONALE
-- ============================================================================